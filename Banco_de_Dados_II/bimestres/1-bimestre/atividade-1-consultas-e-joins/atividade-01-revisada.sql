...
INSERT INTO academia.Endereco (Endereco_ID, Logradouro, Numero, Bairro, CEP)
VALUES
...
INSERT INTO academia.Telefone (Telefone_ID, Telefone_Contato, Telefone_Emergencia)
VALUES
...
INSERT INTO academia.Pessoa_Associada (Pessoa_Associada_ID, Nome, Sobrenome, Endereco_ID, Telefone_ID, Plano_ID)
VALUES
(1, 'Ayla', 'Cunha', 9, 11, 1),
...
INSERT INTO academia.Pessoa_Treinadora (Pessoa_Treinadora_ID, Nome, Sobrenome, Endereco_ID, Telefone_ID)
VALUES
(1, 'Jorge', 'Galvão', 1, 1),
(2, 'Sophia', 'Galvão', 1, 2),
(3, 'Camila', 'Cunha', 2, 3),
...
INSERT INTO academia.Aula (Pessoa_Treinadora_ID, Modalidade_ID, Horario_ID)
VALUES
(1, 1, 1),
(6, 1, 2),
(1, 1, 3),
...
INSERT INTO academia.Treino (Pessoa_Associada_ID, Modalidade_ID, Horario_ID)
VALUES
(1, 3, 1),
(2, 2, 3),
(3, 1, NULL),
...
-- Exercicio 01 --
/* 1 - Crie uma query que retorne uma coluna nomeada como 'Nome Completo' com o nome
completo das pessoas associadas a academia, uma coluna com o logradouro e outra com
o número. Utilize as tabelas Pessoa_Associada e Endereco. */

SELECT CONCAT(PA.Nome, " ", PA.Sobrenome) as Nome_Completo, E.Logradouro, E.Numero
FROM academia.Pessoa_Associada as PA
LEFT JOIN academia.Endereco as E
ON PA.Endereco_ID = E.Endereco_ID
ORDER BY Nome ASC;

-- Exercicio 02 --
/* 2 - Escreva uma query que retorne uma coluna nomeada como 'Nome Completo' com
o nome completo das pessoas associadas a academia, uma coluna com o nome do
plano dessa pessoa. Utilize as tabelas Pessoa_Associada e Plano. */

SELECT CONCAT(PA.Nome, " ", PA.Sobrenome) as Nome_Completo, IFNULL(P.Nome_Plano, "NAO POSSUI") as Plano
FROM academia.Pessoa_Associada as PA
LEFT JOIN academia.Plano as P
ON PA.Plano_ID = P.Plano_ID
ORDER BY Plano DESC;

-- Exercicio 03 --
/* 3 - Escreva uma query que retorne uma coluna nomeada como 'Nome Completo' com
o nome completo das pessoas associadas a academia, uma coluna com telefone e outra
com o cep dessa pessoa. Utilize as tabelas Pessoa_Associada, Telefone e Endereco. */

SELECT CONCAT(PA.Nome, " ", PA.Sobrenome) as Nome_Completo, IFNULL(TEL.Telefone_Contato, "NAO POSSUI") as Contato_Principal, E.Cep
FROM academia.Pessoa_Associada as PA
LEFT JOIN academia.Telefone as TEL
ON PA.Telefone_ID = TEL.Telefone_ID
LEFT JOIN academia.Endereco as E
ON PA.Endereco_ID = E.Endereco_ID
ORDER BY Nome ASC;

-- Exercicio 04 --
/* 4 - Escreva uma query que retorne uma coluna nomeada como 'Nome Completo' com
o nome completo das pessoas associadas a academia, uma coluna com telefone de
contato e outra com o cep dessa pessoa. Utilize as tabelas Pessoa_Associada, Telefone e
Endereco. */

SELECT CONCAT(PA.Nome, " ", PA.Sobrenome) as Nome_Completo, IFNULL(TEL.Telefone_Contato, "NAO POSSUI") as Telefone_Contato, E.Cep
FROM academia.Pessoa_Associada as PA
LEFT JOIN academia.Telefone as TEL
ON PA.Telefone_ID = TEL.Telefone_ID
LEFT JOIN academia.Endereco as E
...
-- Exercicio 05 --
/* 5 - Crie uma query que retorne uma coluna nomeada como 'Nome Completo' com o nome
completo de todas as pessoas associadas a academia, e uma coluna com o telefone de
emergência dessa pessoa, caso a pessoa não tiver telefone de emergência, exiba
'PEDENTE', nomeie essa coluna como 'Telefone de Emergência'. Utilize as tabelas
Pessoa_Associada e Telefone. */

SELECT CONCAT(PA.Nome, " ", PA.Sobrenome) as Nome_Completo, IFNULL(TEL.Telefone_Emergencia, "PENDENTE") as Telefone_Emergencia
FROM academia.Pessoa_Associada as PA
LEFT JOIN academia.Telefone as TEL
ON PA.Telefone_ID = TEL.Telefone_ID
ORDER BY Nome ASC;
...
-- Exercicio 06 --
/* 6 - Crie uma query que retorne duas coluna com os nomes das pessoas associadas que
possuem o mesmo endereço. Porém, não deixe que o mesmo nome se repita na mesma
linha. */

SELECT CONCAT(PA.Nome, " ", PA.Sobrenome) as Nome_Completo, E.Logradouro
FROM academia.Pessoa_Associada as PA
INNER JOIN academia.Endereco as E
ON PA.Endereco_ID = E.Endereco_ID
WHERE PA.Endereco_ID IN (
	SELECT PA.Endereco_ID
    FROM academia.Pessoa_Associada as PA
    GROUP BY PA.Endereco_ID
    HAVING COUNT(PA.Endereco_ID) > 1
) ORDER BY Logradouro ASC;

-- Exercicio 07 --
/* 7 - Crie uma query que retorne duas coluna com os nomes das pessoas treinadoras que
possuem o mesmo endereço. Porém, não deixe que o mesmo nome se repita na mesma
linha. */

SELECT CONCAT(PT.Nome, " ", PT.Sobrenome) as Nome_Completo, E.Logradouro
FROM academia.Pessoa_Treinadora as PT
INNER JOIN academia.Endereco as E
ON PT.Endereco_ID = E.Endereco_ID
WHERE PT.Endereco_ID IN (
	SELECT PT.Endereco_ID
    FROM academia.Pessoa_Treinadora as PT
	GROUP BY PT.Endereco_ID
    HAVING COUNT(PT.Endereco_ID) > 1
) ORDER BY Nome ASC;

-- Exercicio 08 --
/* 8 - Crie uma query que retorne uma coluna com o nome dos planos outra que com
a quantidade de pessoas associadas que possuem esse plano, nomeie como 'Pessoas
Inclusas'. */

SELECT P.Nome_Plano, COUNT(PA.Plano_ID) as Pessoas_Inclusas
FROM academia.Pessoa_Associada as PA
INNER JOIN academia.Plano as P
ON PA.Plano_ID = P.Plano_ID
GROUP BY P.Nome_Plano;

-- Exercicio 09--
/* 9 - Crie uma query que retorne uma coluna nomeada como 'Nome Completo', com o nome
completo das pessoas associadas a academia, e uma coluna com o período dessa pessoa.
Utilize as tabelas Pessoa_Associada, Treino e Horario. */

SELECT CONCAT(PA.Nome, " ", PA.Sobrenome) as Nome_Completo, IFNULL(H.Periodo, "NAO POSSUI") as Periodo
FROM academia.Pessoa_Associada as PA
LEFT JOIN academia.Treino as T
ON T.Pessoa_Associada_ID = PA.Pessoa_Associada_ID
LEFT JOIN academia.Horario as H
ON T.Horario_ID = H.Horario_ID
ORDER BY Nome ASC;

-- Exercicio 10 --
/* 10 - Crie uma query que retorne uma coluna nomeada como 'Nome Completo' com o nome
completo de todas as pessoas associadas a academia e uma coluna com o nome 'Horario',
mostrando o período dessa pessoa. Caso a pessoa não tiver período estipulado, exiba
'LIVRE'. Utilize as tabelas Pessoa_Associada, Aula e Horario. */

SELECT CONCAT(PT.Nome, " ", PT.Sobrenome) as Nome_Completo, IFNULL(H.Periodo, "LIVRE") as Horario
FROM academia.Pessoa_Treinadora as PT
LEFT JOIN academia.Aula as A
