...
exercicio1("1-listaNomesIdades.txt")
...
exercicio2("2-listaNomesIdadesAlturas.txt")
...
exercicio3("3-antigo.txt", "3-novo.txt")
...
exercicio5("4-listaNotasAlunos.txt","4-listaNomesAlunos.txt", "Ana Silva", "9.0 9.0 9.0\n")
...
def exercicio6(arquivoIP):
    ...
    arquivoIpInvalido = open("6-listaIPsInvalidos.txt", 'w+')
    arquivoIpValido = open("6-listaIPsValidos.txt", 'w+')
    ...
    while ip != "":
        ...
        if len(numero_ip) == 4:
            for i in range(4):
                octeto = int(numero_ip[i])

                if i == 0 and not (1 <= octeto <= 255):
                    lista_ip_invalido.append(ip)
                    break
                else:
                    if i != 0 and not (0 <= octeto <= 255):
                        lista_ip_invalido.append(ip)
                        break
                    elif i == 3 and (0 <= octeto <= 255):
                        lista_ip_valido.append(ip)                  
        else:
            lista_ip_invalido.append(ip)  
        ip = lista_ip.readline()
        
    arquivoIpInvalido.writelines(lista_ip_invalido)
    arquivoIpValido.writelines(lista_ip_valido)
    
    arquivoIpInvalido.close()
    arquivoIpValido.close()
    lista_ip.close()

exercicio6("6-listaIPs.txt")
